...
namespace BIMS_dan
{
    public partial class controllerResidents : UserControl
    {
        ...
        private async void ControllerAddNewResidentModal_ResidentAdded(object sender, EventArgs e)
        {
            Database db = new Database();
            await db.RefreshResidentsTableAsync(this.residentsTable);
        }
        ...
        private void AdjustDataGridViewImageColumnSize()
        {
            ...
            residentsTable.RowTemplate.Height = cellSize; // Adjust row height to match the image size
            residentsTable.Columns["picture"].Width = cellSize; // Adjust column width to match the image size

            residentsTable.CellFormatting += new DataGridViewCellFormattingEventHandler(residentsTable_CellFormatting);
        }

        private void residentsTable_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
        {
            if (e.ColumnIndex == residentsTable.Columns["picture"].Index && e.Value != null)
            {
                try
                {
                    ...
                    using (var ms = new MemoryStream((byte[])e.Value))
                    {
                        ...
                        e.Value = ResizeImage(originalImage, 50, 50); // Resize and set the new image as cell value
                    }
                }
                catch
                {
