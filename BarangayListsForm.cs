...
public partial class BarangayListsForm : Form {
  ...
  private void addBarangayButton_Click(object sender, EventArgs e) {
    using (Form modalForm = new Form()) {
      ControllerAddNewBarangayModal ControllerAddNewBarangayModal =
          new ControllerAddNewBarangayModal();
  ...
  private void BarangayListsForm_Load(object sender, EventArgs e) {
    ...
    // '_dan_bimsBarangayDataset.Barangays' table. You can move, or remove it,
    // as needed.
  ...
  barangaysDatatable_CellContentClick(object sender,
    ...
    if (e.RowIndex >= 0) {
      ...
      var cellValue = dgv.Rows[e.RowIndex]
                          .Cells["barangayLogoDataGridViewImageColumn"]
                          .Value;
  ...
  private void barangaysDatatable_SelectionChanged(object sender, EventArgs e) {
    ...
    if (barangaysDatatable.SelectedRows.Count > 0) {
      // Assuming 'barangayNameDataGridViewTextBoxColumn' is the name of the
      // column Get the value from the first selected row
      string selectedBarangayName =
          barangaysDatatable.SelectedRows[0]
              .Cells["barangayNameDataGridViewTextBoxColumn"]
              .Value.ToString();
  ...
  private async void deleteBarangayButton_Click(object sender, EventArgs e) {
    if (barangaysDatatable.SelectedRows.Count > 0) {
      var confirmResult = MessageBox.Show(
          "Are you sure you want to delete this barangay? All residents in " +
          "this Barangay will also be deleted.",
          "Confirm Delete", MessageBoxButtons.YesNo);
      if (confirmResult == DialogResult.Yes) {
        int idColumnIndex =
            barangaysDatatable.Columns["barangayIDDataGridViewTextBoxColumn"]
  ...
  private void editBarangayButton_Click(object sender, EventArgs e) {
    ...
    this.barangaysDatatable.CellClick += new DataGridViewCellEventHandler(
        this.barangaysDatatable_CellContentClick);
    using (Form modalForm = new Form()) {
      ControllerEditBarangayModal ControllerEditBarangayModal =
          new ControllerEditBarangayModal();
  ...
  private async void searchBarangayText_TextChanged(object sender,
                                                    EventArgs e) {
    BarangayDB db = new BarangayDB();
    TextBox searchBarangayText = sender as TextBox;
    if (searchBarangayText != null) {
      try {
        // Assuming SearchBarangayAsync is a method of BarangayDB and returns a
        // DataTable
        DataTable searchResults =
            await db.SearchBarangayAsync(searchBarangayText.Text);
        barangaysDatatable.DataSource = searchResults;
      } catch (Exception ex) {
        MessageBox.Show($"Error searching barangay: {ex.Message}",
